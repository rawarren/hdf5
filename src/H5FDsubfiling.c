...
static const H5FD_class_mpi_t H5FD_subfiling_g = {
    ...
    H5FD__subfiling_mpi_rank,
    H5FD__subfiling_mpi_size,
    H5FD__subfiling_communicator,
...
static herr_t H5FD__init_package(void) {
  ...
  if (H5FD_subfiling_init() < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL,
                "unable to initialize subfiling VFD")
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5FD_subfiling_init(void) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_fapl_subfiling(hid_t fapl_id, H5FD_subfiling_config_t *fa) {
...
static herr_t fapl__get_subfiling_defaults(H5FD_subfiling_config_t *fa) {
  herr_t ret_value = SUCCEED;
  char *envValue = NULL;
 ...
 */
static void *H5FD__subfiling_fapl_copy(const void *_old_fa) {
  const H5FD_subfiling_config_t *old_fa =
      (const H5FD_subfiling_config_t *)_old_fa;
  ...
  new_fa =
      (H5FD_subfiling_config_t *)H5MM_malloc(sizeof(H5FD_subfiling_config_t));
  if (new_fa == NULL) {
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
  }
 ...
 */
static H5FD_t *H5FD__subfiling_open(const char *name, unsigned flags,
                                    hid_t subfiling_fapl_id, haddr_t maxaddr) {
  H5FD_subfiling_t *file_ptr = NULL; /* Subfiling VFD info */
  const H5FD_subfiling_config_t *config_ptr =
      NULL;                    /* Driver-specific property list */
  ...
  file_ptr->sf_file =
      H5FD_open(name, flags, config_ptr->common.ioc_fapl_id, HADDR_UNDEF);
  if (!file_ptr->sf_file)
  ...
  if (strncmp(driver->name, "ioc", 3) == 0) {
    ...
    H5FD_subfiling_t *ioc_file = file_ptr->sf_file;
    ...
    if (MPI_SUCCESS ==
        MPI_Bcast(&inode_id, 1, MPI_UNSIGNED_LONG_LONG, 0, MPI_COMM_WORLD)) {
    ...
    if (H5FD__open_subfiles((void *)&file_ptr->fa, inode_id, file_ptr->fd,
                            ioc_flags) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_query(const H5FD_t *_file,
                                    unsigned long *flags /* out */) {
  const H5FD_subfiling_t *file =
      (const H5FD_subfiling_t *)_file; /* subfiling VFD info */
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__subfiling_get_eoa(const H5FD_t *_file,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_set_eoa(H5FD_t *_file,
                                      H5FD_mem_t H5_ATTR_UNUSED type,
                                      haddr_t addr) {
  H5FD_subfiling_t *file_ptr = (H5FD_subfiling_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__subfiling_get_eof(const H5FD_t *_file,
                                       H5FD_mem_t H5_ATTR_UNUSED type) {
  const H5FD_subfiling_t *file = (const H5FD_subfiling_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_get_handle(H5FD_t *_file,
...
/*
 * A simple utility function to count the number of non-zero
 * values in a vector.  Used by the read and write operations
 * to construct vectors to pass to the stacked IOC VFD.
 */
static inline int get__ioc_count(int length, int64_t *data_size) {
  int k, count = 0;
  for (k = 0; k < length; k++) {
    if (data_size[k] > 0)
      count++;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_read(H5FD_t *_file,
                                   H5FD_mem_t H5_ATTR_UNUSED type,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_read_vector(H5FD_t *_file, hid_t dxpl_id,
                                          uint32_t count, H5FD_mem_t types[],
                                          haddr_t addrs[], size_t sizes[],
                                          void *bufs[] /* out */) {
  H5FD_subfiling_t *file = (H5FD_subfiling_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_write_vector(H5FD_t *_file, hid_t dxpl_id,
                                           uint32_t count, H5FD_mem_t types[],
                                           haddr_t addrs[], size_t sizes[],
                                           const void *bufs[] /* in */) {
  H5FD_subfiling_t *file_ptr = (H5FD_subfiling_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_truncate(H5FD_t *_file,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD__get_file_ino(const char *name, uint64_t *st_ino) {
  herr_t ret_value = SUCCEED; /* Return value */
  h5_stat_t sb;
...
static herr_t H5FD__subfiling_ctl(H5FD_t *file, uint64_t op_code,
...
static herr_t create__simple_vector(hid_t file_space_id, void *memDataBuf,
                                    haddr_t addrBase, hssize_t elements,
                                    size_t type_extent, hssize_t *vlen,
                                    haddr_t **_offsets, hsize_t **_blocklens,
                                    void ***_bufs) {
...
static herr_t create__vector_from_hyperslab(
    hid_t file_space_id, void *memDataBuf, haddr_t addrBase, size_t type_extent,
  ...
  if (n_blocks > H5S_MAX_RANK) {
    ...
    if ((strides = (hsize_t *)malloc((size_t)n_blocks * sizeof(hsize_t))) ==
        NULL) {
      perror("unable to allocate storage for vector creation");
      return -1;
    }
    if ((counts = (hsize_t *)malloc((size_t)n_blocks * sizeof(hsize_t))) ==
        NULL) {
      perror("unable to allocate storage for vector creation");
      return -1;
    }
  ...
  if (*vlen < n_blocks) {
    if (offsets) {
      offsets =
          (haddr_t *)realloc(offsets, ((size_t)n_blocks * sizeof(haddr_t)));
    } else {
    ...
    if (blocklens) {
      blocklens =
          (hsize_t *)realloc(blocklens, ((size_t)n_blocks * sizeof(hsize_t)));
    } else {
  ...
  if ((ret_value = H5Sget_regular_hyperslab(file_space_id, (hsize_t *)offsets,
                                            strides, counts, blocklens)) < 0) {
...
static herr_t check__dims(int ndims, hsize_t *mem_dims, hsize_t *file_dims,
                          int *diff_index) {
  int i;
  herr_t ret_value = SUCCEED;
  for (i = 0; i < ndims; i++) {
...
static haddr_t get__data_offset(int mpi_rank, int mpi_size, size_t dtype_extent,
...
static haddr_t get__base_offset(int mpi_rank, int mpi_size, size_t dtype_extent,
...
herr_t H5FD__dataset_write_contiguous(
  ...
  if (num_elem_file > 0) {
    ...
    case H5S_SEL_POINTS: {
      ...
      printf(
          "[%d] H5S_SEL_POINTS - num_elem_file: %lld: UNSUPPORTED (for now)\n",
    ...
    case H5S_SEL_ALL: {
      ...
      if (num_elem_mem > 0) {
        ...
        if (status > 0) {
          if (create__simple_vector(file_space_id, buf, rank_baseAddr,
                                    num_elem_mem, dtype_extent, &sf_vlen,
                                    &sf_offsets, &sf_sizes, &sf_bufs) < 0) {
...
herr_t H5FD__dataset_read_contiguous(hid_t h5_file_id, haddr_t dataset_baseAddr,
                                     size_t dtype_extent, int mpi_rank,
                                     int mpi_size, void *_dset,
...
void manage_client_logfile(int client_rank, int flag_value) {
#ifndef NDEBUG
  if (flag_value) {
    char logname[64];
