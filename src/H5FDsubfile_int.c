 ...
 */
int
H5FD__open_subfiles(void *_config_info, int64_t h5_file_id, int flags)
{
    int                  status;
    ...
    if (config_info->magic == H5FD_IOC_FAPL_T_MAGIC) {
        HDputs("using IO Concentrators");
    }
    else if (config_info->magic == H5FD_SUBFILING_FAPL_T_MAGIC) {
        HDputs("NOT using IO Concentrators");
    }
    else {
...
int
H5FD__read_subfiles(int64_t context_id, haddr_t addr, size_t size, const void *buf)
{
    subfiling_context_t *sf_context = get_subfiling_object(context_id);
    assert(sf_context != NULL);
    /* We might want to validate that the size doesn't exceed 2GB.
     * If it does, we can break the single request into a vectored write.
     */
    if (size < twoGIG_LIMIT) {
		return sf_read_independent(context_id, addr, size, 1, buf);
	}
    /* FIXME: convert into the vectored approach with each element
     * being 1GB or less.
     */
    return -1;
}
...
int
H5FD__write_subfiles(int64_t context_id, haddr_t addr, size_t size, const void *buf)
{
    subfiling_context_t *sf_context = get_subfiling_object(context_id);
    assert(sf_context != NULL);
    /* We might want to validate that the size doesn't exceed 2GB.
     ...
     */
    if (size < twoGIG_LIMIT) {
		return sf_write_independent(context_id, addr, size, 1, buf);
	}
    /* FIXME: convert into the vectored approach with each element
     * being 1GB or less.
     */
