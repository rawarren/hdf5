 ...
 * 
 * Purpose:    Given blah, blah,
 *             return an instance of a driver_info_t structure (shown below)
 *                (byte)     (byte)    (byte)     (byte)
 *             +----------+----------+----------+----------+
 *             | Version  | //////// | //////// | //////// |
 *             +----------+----------+----------+----------+
...
create_subfiling_ioc_fapl(const char *_basename, struct subfilingtest_filenames *names)
{
    ...
    hid_t                    under_fapl = H5I_INVALID_HID;
    ...
    if (subfiling_conf.require_ioc) {
        ...
        if (H5Pset_fapl_sec2(under_fapl) == FAIL) {
            TEST_ERROR;
        }
	}

    /* Assign the IOC fapl as the underlying VPD */
	subfiling_conf.common.ioc_fapl_id = under_fapl;
...
test_basic_dataset_write(void)
{
    ...
    int         buf_size  = DATABUFFER_SIZE * DATABUFFER_SIZE;
    int         i = 0;
    int         j = 0;
    int         k = 0;
     ...
     */
    check = (int *)HDmalloc(DATABUFFER_SIZE * DATABUFFER_SIZE * sizeof(int));
    if (NULL == check) {
        TEST_ERROR;
    }
